...
public:
  ...
  static const inline cutter_power_t cpwr_to_upwr(const cutter_cpower_t cpwr) { // STARTUP power to Unit power
    const cutter_power_t upwr = ( DeltaMachineMode::isCNC() ? (
        // Spindle configured values are in RPM
        #if SPINDLE_CUTTER_UNIT_IS(RPM)
          cpwr                            // to RPM
        #elif SPINDLE_CUTTER_UNIT_IS(PERCENT)     // to PCT
          cpwr_to_pct(cpwr)
        #else                             // to PWM
        ...
        #if LASER_CUTTER_UNIT_IS(PWM255)
          PCT_TO_PWM(cpwr)
        #else
          cpwr                            // to RPM/PCT
        #endif
  ...
  static const cutter_power_t mpower_min() { return cpwr_to_upwr(DeltaMachineMode::get_min_power()); }
  static const cutter_power_t mpower_max() { return cpwr_to_upwr(DeltaMachineMode::get_max_power()); }
  ...
  #if ENABLED(MARLIN_DEV_MODE)
    static inline void refresh_frequency() { set_pwm_frequency(pin_t(LASER_PWM_PIN), frequency); set_pwm_frequency(pin_t(SPINDLE_PWM_PIN), frequency);}
  #endif
  ...
  FORCE_INLINE static void set_power(const uint8_t upwr) { power = upwr; refresh(); }
    ...
    static inline uint8_t upower_to_ocr(const cutter_power_t upwr) {
        return ( DeltaMachineMode::isCNC() ? (
          #if SPINDLE_CUTTER_UNIT_IS(PWM255)
            uint8_t(upwr)
          #elif SPINDLE_CUTTER_UNIT_IS(PERCENT)
            pct_to_ocr(upwr)
          #else
          ...
          #if LASER_CUTTER_UNIT_IS(PWM255)
            uint8_t(upwr)
          #elif LASER_CUTTER_UNIT_IS(PERCENT)
            pct_to_ocr(upwr)
          #else
            uint8_t(pct_to_ocr(cpwr_to_pct(upwr)))
          #endif
    ...
    static inline cutter_power_t power_to_range(const cutter_power_t pwr) {
        return power_to_range(pwr, ( DeltaMachineMode::isCNC() ? (
          #if SPINDLE_CUTTER_UNIT_IS(PWM255)
            0
          #elif SPINDLE_CUTTER_UNIT_IS(PERCENT)
            1
          #elif SPINDLE_CUTTER_UNIT_IS(RPM)
            2
          #else
            #error "???"
          #endif
    ...
    static inline cutter_power_t power_to_range(const cutter_power_t pwr, const uint8_t pwrUnit) {
      ...
      switch (pwrUnit) {
        ...
        case 2:                                                 // RPM
          upwr = cutter_power_t(
              (pwr < DeltaMachineMode::get_min_power()) ? DeltaMachineMode::get_min_power()         // Use minimum if set below
            : (pwr > DeltaMachineMode::get_max_power()) ? DeltaMachineMode::get_max_power()         // Use maximum if set above
  ...
  static inline void disable() { isReady = false; set_enabled(false); }
    ...
    static inline void enable_with_dir(const bool reverse) {
      ...
      if (menuPower)
        ...
        menuPower = cpwr_to_upwr(DeltaMachineMode::get_startup_power());
      unitPower = menuPower;
    ...
    static inline void set_inline_enabled(const bool enable) {
      if (enable)
        inline_power(cpwr_to_upwr(DeltaMachineMode::get_startup_power()));
      else {
        isReady = false;
